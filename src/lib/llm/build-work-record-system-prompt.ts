...
type BuildWorkRecordSystemPromptParams = {
  executionScope: {
    internRunId: string
    projectId: string
    recordId: string
...
export const buildWorkRecordSystemPrompt = ({
  ...
  taskTitle,
  previousExecutions,
}: BuildWorkRecordSystemPromptParams) =>
  `
...
under ${backendConfig.BETTER_AUTH_URL}/api/crm/schema.json. The bearer token is available in \`.env.intern\` as \`CRM_BEARER_TOKEN\`.
...
Intern run ID: ${executionScope.internRunId}

Work record ID: ${executionScope.workRecordId}
