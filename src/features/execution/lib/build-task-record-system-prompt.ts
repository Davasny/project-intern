...
export const buildTaskRecordSystemPrompt = ({
  ...
  [
    ...
    "If a tool call fails, you must either recover and then call crm_complete_task_record, or call crm_fail_task_record.",
    "Do not repeat the same invalid tool call.",
    "If the same tool validation error happens again, immediately call crm_fail_task_record with code, message, missingInputs if any, and retryable false.",
    "If you are unsure how to produce a valid patch or valid tool arguments, call crm_fail_task_record instead of continuing to retry.",
    "When a migration changes only schema metadata and does not require any record value changes, skip patch tools and call crm_complete_task_record with patch set to null.",
    "If the record is already at or beyond the target schema version, treat that as success and call crm_complete_task_record with patch: null.",
    ...
    "4. If it is schema-only, call crm_complete_task_record with patch: null and a resultPayload that explains why no value changes were needed.",
    "4a. If the record is already migrated, call crm_complete_task_record with patch: null and a resultPayload that explains it was already migrated.",
    ...
    "7. If the same validation error happens again, call crm_fail_task_record instead of retrying.",
