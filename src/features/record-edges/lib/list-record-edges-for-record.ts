...
type ListRecordEdgesForRecordParams = {
...
export const listRecordEdgesForRecord = async ({
  organizationSlug,
  projectSlug,
  recordId,
  userId,
}: ListRecordEdgesForRecordParams) => {
  ...
  const recordEdges = await db
    ...
    .where(
      and(
        ...
        or(
          and(
            eq(recordEdgeTable.fromProjectId, project.id),
            eq(recordEdgeTable.fromRecordId, recordId),
          ),
          and(
            eq(recordEdgeTable.toProjectId, project.id),
  ...
  const relations = recordEdges.map((recordEdge) => {
    ...
    const perspective = isOutbound ? "outbound" : "inbound"
