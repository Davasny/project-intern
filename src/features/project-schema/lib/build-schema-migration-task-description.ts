...
export const buildSchemaMigrationTaskDescription = ({
  ...
  return [
    ...
    "3. If no record value must change, do not call `crm_propose_patch` or `crm_apply_patch`. Complete the task with `crm_complete_task_record` and `patch: null`.",
    `4. If the record is already at schema v${nextVersion}, treat that as a successful schema-only migration and complete with \`crm_complete_task_record\` and \`patch: null\`.`,
    ...
    "- If you cannot produce a valid patch or valid completion payload, call `crm_fail_task_record` instead of retrying the same invalid tool call.",
    "",
    "## Execution algorithm",
    ...
    "   - Outcome A: no record values must change -> call `crm_complete_task_record` with `patch: null`.",
    "   - Outcome A1: the record is already on the target schema version -> call `crm_complete_task_record` with `patch: null` and explain that it was already migrated.",
    ...
    "- If the same validation error happens again, stop retrying and call `crm_fail_task_record`.",
